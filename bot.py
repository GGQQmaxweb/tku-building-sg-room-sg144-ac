...
class ACController:
    ...
    def set_temperature(self, temp: str):
        ...
        try:
            ...
            return True, f"Successfully set AC temperature to {temp}°C"
        except Exception as e:
            self.authenticated = False
...
@client.tree.command(name="ac", description="Control the SG144 Air Conditioner temperature")
@app_commands.describe(temperature="The target temperature (e.g., 18, 19, 22, 26)")
async def ac_command(interaction: discord.Interaction, temperature: str):
    await interaction.response.defer(thinking=True)
    
    # Run temperature setting (requests is blocking, can run in executor if needed, but for simple bot it's fine)
    success, message = ac_controller.set_temperature(temperature)
